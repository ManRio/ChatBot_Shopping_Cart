...
@pytest.fixture
def camiseta_azul():
    return Product(id=101, name="Camiseta Azul", price=15.99)

@pytest.fixture
def gorra_negra():
    return Product(id=402, name="Gorra Negra", price=9.99)
...
def test_remove_item_removes_product(gorra_negra):
    cart = Cart()
...
def test_set_quantity_raises_if_not_in_cart():
    cart = Cart()
    with pytest.raises(KeyError):
        cart.set_quantity(999, 3)

def test_add_item_invalid_quantity_raises(camiseta_azul):
    cart = Cart()
